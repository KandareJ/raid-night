...
![Door Map](../resources/vog-door.png)
...
There are 5 rounds of oracles. In the first round, three oracles spawn, then four, then five, then six, and finally all seven. The order of the oracles will be shown twice, and then they will spawn. Oracles need to be killed in order.

Spawns are shown below:

![Oracle Map](../resources/vog-templar-oracles-map.png)

Xenophage or a sniper rifle are good for destroying the oracles fast at distance. Three guardians will focus on add clear, and three will worry about the oracles.
...
During the encounter, oracles will spawn like before.

```admonish warning title="Oracle order"
If oracles aren't destroyed in order in time, the entire fireteam gets marked to wipe. Cleanse marked guardians with the relic to remove it.
```
...
6. Could be an elemental type or a weapon archetype. (25%) (Looks like there isn’t one for the weekly raid)
...
![Templar Map](../resources/vog-templar-oracles-map.png)

### Gorgon’s Maze

The Gorgon’s maze is a stealth section. The goal is to make it through to the exit without any member of the fireteam being seen.

```admonish danger title="Stay hidden"
If someone is seen, the entire team wipes.
```

It is possible to kill a Gorgon, but each time one is killed, all the others become stronger. Ideally we won’t be seen at all.
...
![Gorgon's maze map](../resources/vog-gorgons-maze.png)
...
* One guardian opens up the portal to the side the teleported guardians calls out and clears adds  
* The last guardian just clears adds. Focus primarily on adds on the side that the opened portal is on, as we will DPS from that side  
* After the third round of oracles, get in position for DPS

**DPS:**

* Follow the same loadout recommendations from the Templar section  
* One person will get the containment buff and needs to move away from the group. Moving towards Atheon lets the fireteam free you without interrupting DPS too much.  
* Damage the boss

```admonish important title="Containment buff"
Whoever gets the containment buff must separate from the group immediately. Pay attention if you have it.
```

Oracle callouts:  
![Oracle callouts](../resources/vog-atheon-oracles-map.png)
