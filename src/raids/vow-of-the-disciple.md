...
![Acquisition map](../resources/vow-acquisition-map.png)

These symbols will be important throughout the entire raid. The room layout is this:

![Symbols map](../resources/vow-symbols.png)
...
3. Light or dark \- in the room from the previous symbol there will be a glyphkeeper on the light side and another on the dark side. Kill the one that corresponds to this symbol, and write the symbol in chat.
...
```admonish warning title="Pillar damage"
If the pillar gets shot too many times by adds, the whole team wipes.
```

1. Unstoppable champions will spawn every once in a while while defending the pillar. Be sure to have some way to deal with them.  
2. Anti-barrier is helpful for the scorn with the stupid little shield that somehow blocks 90% of my shots.  
3. Only half of the doors will be open at a time. Shooting the crystal floating in the middle of the room (marked as “door control” on the map) will switch which doors are open.
...
Recommend pulse rifle with anti-barrier to clear adds at a distance and to be able to hit the snipers when they put up their shield. There will also be overload champions. We will split up into three teams of two:
...
* Team 2 \- Stunners  
  ...
  * Shoot bees and clear adds between stuns.  
...
On the fourth floor, damage phase starts without any other mechanics.

```admonish warning title="Last plate"
If you don’t kill Caretaker before the last plate deactivates, the team wipes.
```
...
```admonish warning title="Pervading darkness"
A 1:15 countdown starts at the beginning of each room. If it hits zero, we wipe. Deposit all relics in the next room to stop it, or reset the timer to 1:14 by killing taken knights.
```
...
#### Team One - Add Clear
The first team of two is just add clear. Most of the adds we need to clear will be at range, so submachine guns and glaives are probably not the way to go here. Prioritize killing glyphkeepers as soon as they spawn. There will be one on each side.
...
After second split, the pillar caller should look down at the six pillars and call out which two contain the correct symbol for the runners to deposit at.

#### Team Four - Buff Holder
The buff holder has an easy job. Once the encounter starts, they shoot the crystal above Rhulk to get the leeching buff. They then stand on the plate and call out "first split". When "second split" is called out, they shoot the buff on the left. When both glyphkeepers are dead, they call out the symbols they see. They can then just clear adds and avoid Rhulk's laser. After the runners dunk, the buff holder should be standing on the plate and call out "first split" to restart their cycle.
...
#### Non-glaive shooters
Do not shoot the glaive. Call out the location to dunk when the symbol appears. When Rhulk's weak spots appear, shoot them. Don't shoot the glaive. Clear adds.

#### All
After all of Rhulk's 4 weak spots are broken, damage phase begins. Kill Rhulk until he is immune. Once damage is over, run back to the plate where the Rhulk encounter began.

```admonish danger title="Get back in time"
If you don't get there in time, Rhulk's forcefield will shoot you off the edge.
```
